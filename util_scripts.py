...
####import config_celebA as config
import config
import misc
import tfutil
import train
...
#----------------------------------------------------------------------------
# Generate interpolated images between 2 latent representation.
# To run, uncomment the appropriate line in config.py and launch train.py.

def generate_fake_interpolate_images(run_id, snapshot=None, grid_size=[1,1], num_pngs=1, image_shrink=1, png_prefix=None, random_seed=1000, minibatch_size=8, middle_img=10):
    network_pkl = misc.locate_network_pkl(run_id, snapshot)
    if png_prefix is None:
        png_prefix = misc.get_id_string_for_network_pkl(network_pkl) + '-'
    random_state = np.random.RandomState(random_seed)
    ...
    result_subdir = misc.create_result_subdir(config.result_dir, config.desc)
    for png_idx in range(num_pngs):
        latents=misc.random_latents(middle_img+2, Gs, random_state=random_state)
        from_to_tensor = latents[middle_img+1] - latents[0]
        from_z=latents[0]
        #between_x_list = [from_x]
        counter=1
        for alpha in np.linspace(0, 1, middle_img + 1):
    ...
    #png_prefix=0
        ...
        misc.save_image_grid(images, os.path.join(result_subdir, '%s%06d.png' % (png_prefix, png_idx)), [0,255], grid_size_1)
    '''
    for png_idx in range(num_pngs):
        print('Generating png %d / %d...' % (png_idx, num_pngs))
        latents = misc.random_latents(np.prod(grid_size), Gs, random_state=random_state)
    ...
    open(os.path.join(result_subdir, '_done.txt'), 'wt').close()
    '''
def generate_fake_interpolate_midle_images(run_id, snapshot=None, grid_size=[1,1], num_pngs=1, image_shrink=1, png_prefix=None, random_seed=1000, minibatch_size=8, middle_img=10):
    network_pkl = misc.locate_network_pkl(run_id, snapshot)
    if png_prefix is None:
        png_prefix = misc.get_id_string_for_network_pkl(network_pkl) + '-'
    random_state = np.random.RandomState(random_seed)
    ...
    for png_idx in range(num_pngs):
        ...
        #between_x_list = [from_x]
        counter=0
        for alpha in np.linspace(-0.5, 0.5, middle_img + 2):#np.linspace(0, 1, middle_img + 1):
            print('alpha: ',alpha ,'counter= ',counter)
            between_z = from_z + alpha * from_to_tensor
            latents[counter]=between_z
            counter+=1
    ...
    #png_prefix=0
        ...
        misc.save_image_grid(images[1:,:,:,:], os.path.join(result_subdir, '%s%06d.png' % (png_prefix, png_idx)), [0,255], grid_size_1)
    '''
    for png_idx in range(num_pngs):
        print('Generating png %d / %d...' % (png_idx, num_pngs))
        latents = misc.random_latents(np.prod(grid_size), Gs, random_state=random_state)
...
def generate_interpolation_video(run_id, snapshot=None, grid_size=[1,1], image_shrink=1, image_zoom=1, duration_sec=60.0, smoothing_sec=1.0, mp4=None, mp4_fps=30, mp4_codec='libx265', mp4_bitrate='16M', random_seed=1000, minibatch_size=8):
    ...
    print('shape is like:',shape)
    all_latents = random_state.randn(*shape).astype(np.float32)
    print('sigma:',[smoothing_sec * mp4_fps] + [0] * len(Gs.input_shape))
