...
result_dir = '/home/newmoon/GAN/CapsGAN/Generator_Results/LSUN_dog'#/home/newmoon/GAN/CapsGAN/Generator_Results/LSUN_res'
...
#64,32,16,8,8,4
desc        = 'pgan_G_caps_cs_64_64_64_64_32_32_32_D_paper_it20_resume2_'#_justtest'#continu1_G_paper_caps_64_64_64_64_32_32_32_0__D_paper_minibatch_rep4_latent512_withIter20sqrt_fue_false'#_dataset100k'# _caps32_16_8_4_2_2_D_paper_capsconv_capsdense32_16_8_4_2_2_latent128_D_repeats1'                                        # Description string included in result subdir name.
...
train       = EasyDict(func='train.train_progressive_gan',resume_run_id=6,resume_kimg=9800)#,minibatch_repeats=1,resume_run_id=0,resume_kimg=5000)#,D_repeats=4)  # Options for main training func.
...
desc += '-lsun-dog-100k';            dataset = EasyDict(tfrecord_dir='lsun-dog-100k');            train.mirror_augment = True
...
desc += '-preset-v2-4gpus'; num_gpus = 4; sched.minibatch_base = 16; sched.minibatch_dict = {4: 512, 8: 256, 16: 128, 32: 64, 64: 32, 128: 12}; sched.G_lrate_dict = {256: 0.0015, 512: 0.002, 1024: 0.003}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 40000
...
##train = EasyDict(func='util_scripts.generate_fake_images', run_id=10, num_pngs=1000); num_gpus = 1; desc = 'fake-images-' + str(train.run_id)
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=3, log='metric-swd-16k.txt', metrics=['swd'], num_images=16384, real_passes=2); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=3, log='metric-fid-50k.txt', metrics=['fid'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
