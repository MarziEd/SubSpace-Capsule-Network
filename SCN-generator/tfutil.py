...
def call_func_by_name(*args, func=None, **kwargs): #ORG
    assert func is not None
    return import_obj(func)(*args, **kwargs)

#def call_func_by_name( func=None,*args, **kwargs):
#    #assert func is not None
#    print("***call_func_by_name", func)
#    return import_obj(func)(*args, **kwargs)
...
class Network:
    ...
    def get_output_for(self, *in_expr, return_as_list=False, **dynamic_kwargs):
        ###print('dynamic kwarg in get_output_for',dynamic_kwargs)
    ...
    '''
    # Get TensorFlow expression(s) for the output(s) of this network, given the inputs.
    def get_output_for(self,  return_as_list=False, *in_expr, **dynamic_kwargs):
        assert len(in_expr) == self.num_inputs
        all_kwargs = dict(self.static_kwargs)
        all_kwargs.update(dynamic_kwargs)
        with tf.variable_scope(self.scope, reuse=True):
