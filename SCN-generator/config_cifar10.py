...
result_dir = '/home/newmoon/GAN/CapsGAN/Generator_Results/CIFAR10_res'
...
desc        = 'pgan_G_paper_caps_half_16_16_16_16_D_paper_capsconv_capsdense_latent128_iteration20sqrt_minibatch_repeats_1'                                        # Description string included in result subdir name.
random_seed = 100#0                                          # Global random seed.
dataset     = EasyDict()                                    # Options for dataset.load_dataset().
train       = EasyDict(func='train.train_progressive_gan',minibatch_repeats=1)  # Options for main training func.
G           = EasyDict(func='networks.G_paper_caps_half',fmap_base=4096,fmap_max=128,latent_size=128)#512,structure='recursive')             # Options for generator network.
D           = EasyDict(func='networks.D_paper',fmap_base=4096,fmap_max=128)#,structure='recursive')             # Options for discriminator network.
#D           = EasyDict(func='networks.D_paper_capsconv_capsdense',fmap_base=4096,fmap_max=128,structure='recursive')             # Options for discriminator network.
G_opt       = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8) # Options for generator optimizer.
D_opt       = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8) # Options for discriminator optimizer.
G_loss      = EasyDict(func='loss.G_wgan_acgan')            # Options for generator loss.
D_loss      = EasyDict(func='loss.D_wgangp_acgan',wgan_target=750.0)          # Options for discriminator loss.
sched       = EasyDict(lod_training_kimg       = 400, lod_transition_kimg     = 400  )                                    # Options for train.TrainingSchedule.
...
desc += '-preset-v2-1gpu'; num_gpus = 1 ; sched.minibatch_base = 64; sched.minibatch_dict = {4: 128, 8: 128, 16: 128, 32: 64}; train.total_kimg = 30000
...
#desc += '-nogrowing'; sched.lod_initial_resolution = 32; sched.lod_training_kimg = 0; sched.lod_transition_kimg = 0; train.total_kimg = 10000 #sched.lod_initial_resolution = 1024
...
train = EasyDict(func='util_scripts.evaluate_metrics', run_id=10, log='metric-fid-50k.txt', metrics=['fid'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=20, log='metric-is-50k.txt', metrics=['is'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
