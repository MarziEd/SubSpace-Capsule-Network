...
import config
print("This is the config for this run",config)
#####import config_celebA as config

print('******************** I am on stone and config is : ',config.dataset['tfrecord_dir'])
import tfutil
import dataset
import misc
...
def process_reals(x, lod, mirror_augment, drange_data, drange_net):
    print('*****************************************shape of data',tf.shape(x))
...
def train_progressive_gan(
    ...
    with tf.name_scope('Inputs'):
        ...
        #print('$$$$$$$$$$$$$$$$$$$$$$minibatch_in',minibatch_in,'minibatch_split',minibatch_split)
        reals, labels   = training_set.get_minibatch_tf()
        reals_split     = tf.split(reals, config.num_gpus)
        labels_split    = tf.split(labels, config.num_gpus)
    G_opt = tfutil.Optimizer(name='TrainG', learning_rate=lrate_in, **config.G_opt)
    D_opt = tfutil.Optimizer(name='TrainD', learning_rate=lrate_in, **config.D_opt)
    print('Building TensorFlow graph is done ...')
    ...
    print('Setting up snapshot image grid is done...')
    if config.G['func']=='network.G_paper_DCGAN':
        grid_fakes = Gs.run(grid_latents, grid_labels,False, minibatch_size=sched.minibatch//config.num_gpus)
    else:
...
if __name__ == "__main__":
    ...
    print('Running %s()...' % config.train['func']) #org
    ####print('Running %s()...' % config.train['func'])

    tfutil.call_func_by_name(**config.train) #org
    #tfutil.call_func_by_name(func=config,**config.train)
    print('Exiting...')
