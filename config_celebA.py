...
data_dir = '/home/newmoon/Datasets/'#cifar10_tf/'#'datasets'
...
desc        = 'pgan_G_paper_caps128_128_128_64_32_0_half_score_D_paper_latent128_D_minibatch_rep4_numItr20'#'pgan_G_paper_caps64_32_32_32_16_16_D_paper_latent128_D_minibatch_rep4_iteration40sqrt'                                        # Description string included in result subdir name.
random_seed = 1000                                          # Global random seed.
dataset     = EasyDict()                                    # Options for dataset.load_dataset().
train       = EasyDict(func='train.train_progressive_gan',minibatch_repeats=4)#,resume_run_id=8,resume_kimg=20000)#,D_repeats=4)  # Options for main training func.
G           = EasyDict(func='networks.G_paper_caps_half_score',fmap_base=2048,fmap_max=512,latent_size=128) #networks.G_paper_caps    ,structure='recursive')             # Options for generator network.
...
desc += '-preset-v2-2gpus'; num_gpus = 2; sched.minibatch_base = 16; sched.minibatch_dict = {4: 256, 8: 256, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8}; sched.G_lrate_dict = {512: 0.0015, 1024: 0.002}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 40000#12000
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=20, log='metric-swd-16k.txt', metrics=['swd'], num_images=16384, real_passes=2); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=20, log='metric-fid-50k.txt', metrics=['fid'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=20, log='metric-msssim-20k.txt', metrics=['msssim'], num_images=20000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
