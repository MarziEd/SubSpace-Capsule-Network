...
#64,32,16,8,8,4
desc        = 'pgan_continu16_G_paper_caps_half128_128_64_64_64R0_D_paper_minibatch_rep4_latent512_withIter20sqrt_fues_false_mirror_true'# _caps32_16_8_4_2_2_D_paper_capsconv_capsdense32_16_8_4_2_2_latent128_D_repeats1'                                        # Description string included in result subdir name.
random_seed = 120#1000                                          # Global random seed.
dataset     = EasyDict(resolution=128)                                    # Options for dataset.load_dataset().
train       = EasyDict(func='train.train_progressive_gan',minibatch_repeats=4,resume_run_id=16,resume_kimg=20000)#40000)#,D_repeats=4)  # Options for main training func.
G           = EasyDict(func='networks.G_paper_caps_half',fmap_base=2048,fmap_max=512,latent_size=512,fused_scale=False,structure='recursive')#'networks.G_paper_caps'#,resolution=128) #   networks.G_paper_caps networks.G_paper_caps    ,structure='recursive')             # Options for generator network.
D           = EasyDict(func='networks.D_paper',fmap_base=2048,fmap_max=512,structure='recursive')#,resolution=128)#, networks.D_paper_capsconv_capsdense structure='recursive') 
...
#desc += '-lsun-bedroom-256';        dataset = EasyDict(tfrecord_dir='lsun-bedroom256-100k');        train.mirror_augment = True # lsun-bedroom-100k   bedroom_train_tf_128 lsun-bedroom256-100k
desc += '-lsun-bedroom';        dataset = EasyDict(tfrecord_dir='bedroom_train_tf_128');        train.mirror_augment = False
...
desc += '-preset-v2-2gpus'; num_gpus = 2; sched.minibatch_base = 16; sched.minibatch_dict = {4: 256, 8: 256, 16: 128, 32: 64, 64: 32, 128: 16, 256: 16}; sched.G_lrate_dict = { 512: 0.0015, 1024: 0.002}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 80000#12000
#desc += '-preset-v2-4gpus'; num_gpus = 4; sched.minibatch_base = 16; sched.minibatch_dict = {4:512, 8: 256, 16: 128, 32: 64, 64: 32, 128: 16, 256:8}; train.mirror_augment=True;train.total_kimg = 40000#sched.G_lrate_dict = {256: 0.0015, 512: 0.002, 1024: 0.003}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 12000
...
#train = EasyDict(func='util_scripts.generate_fake_images', run_id=8, num_pngs=1000); num_gpus = 1; desc = 'fake-images-' + str(train.run_id)
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=18, log='metric-swd-16k.txt', metrics=['swd'], num_images=16384, real_passes=2); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=21, log='metric-fid-50k.txt', metrics=['fid'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
...
train = EasyDict(func='util_scripts.evaluate_metrics', run_id=17, log='metric-msssim-20k.txt', metrics=['msssim'], num_images=20000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
