...
data_dir = '/home/newmoon/Datasets/'#cifar10_tf/'#'datasets' celebA 
#data_dir = '/home/newmoon/Datasets/LSUN_tf_pgan/' #LSUN 
...
desc        = 'pgan_Gulja_G_paper_D_paper_latent128_D_minibatch_rep1_'#'pgan_G_paper_caps64_32_32_32_16_16_D_paper_latent128_D_minibatch_rep4_iteration40sqrt'                                        # Description string included in result subdir name.
random_seed = 1000                                          # Global random seed.
dataset     = EasyDict()                                    # Options for dataset.load_dataset().
train       = EasyDict(func='train.train_progressive_gan',minibatch_repeats=1,D_repeats=5,G_smoothing=0.999**20)#,adam_beta2=0.9)#,resume_run_id=0,resume_kimg=14040)#,D_repeats=4)  # Options for main training func.
G           = EasyDict(func='networks.G_paper',fmap_base=2048,fmap_max=512,latent_size=512)#128) #networks.G_paper_caps    ,structure='recursive')             # Options for generator network.
D           = EasyDict(func='networks.D_paper',fmap_base=2048,fmap_max=512)#,structure='recursive') #networks.D_paper_capsconv_capsdense
G_opt       = EasyDict(beta1=0.0, beta2=0.9, epsilon=1e-8) # Options for generator optimizer.
D_opt       = EasyDict(beta1=0.0, beta2=0.9, epsilon=1e-8) # Options for discriminator optimizer.
G_loss      = EasyDict(func='loss.G_wgan_acgan')            # Options for generator loss.
D_loss      = EasyDict(func='loss.D_wgangp_acgan',wgan_target=1.0,wgan_epsilon=0.0)          # Options for discriminator loss.
...
desc += '-preset-v2-2gpus'; num_gpus = 2; sched.minibatch_base = 64; train.total_kimg = 20000# sched.minibatch_dict = {4: 256, 8: 256, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8}; sched.G_lrate_dict = {512: 0.0015, 1024: 0.002}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 20000#12000
#desc += '-preset-v2-4gpus'; num_gpus = 4; sched.minibatch_base = 16; sched.minibatch_dict = {4: 512, 8: 256, 16: 128, 32: 64, 64: 32, 128: 16}; train.mirror_augment=True;train.total_kimg = 40000#sched.G_lrate_dict = {256: 0.0015, 512: 0.002, 1024: 0.003}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 12000
...
desc += '-nogrowing'; sched.lod_initial_resolution = 128; sched.lod_training_kimg = 0; sched.lod_transition_kimg = 0; train.total_kimg = 20000
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=15, log='metric-swd-16k.txt', metrics=['swd'], num_images=16384, real_passes=2); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
...
train = EasyDict(func='util_scripts.evaluate_metrics', run_id=15, log='metric-fid-50k.txt', metrics=['fid'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
